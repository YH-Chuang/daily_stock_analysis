...
| `REPORT_LANGUAGE` | Default output language for reports and Agent Chat: `zh` (default Simplified Chinese) / `zh-tw` (Traditional Chinese, Taiwan usage) / `en` (English) / `ko` (Korean); also updates prompt instructions, templates, notification fallbacks, fixed copy in the Web report view, and Ask Stock replies that omit `context.report_language`. `zh-tw` uses Taiwan-localized terminology (e.g. 利多, 類股, 本益比) and maintains its own structural labels separate from `zh`. `ko` reuses the English structural scaffolding and constrains the model to Korean output via an output-language directive; notifications render localized labels by report language. The bundled `00-daily-analysis.yml` already maps this variable, so setting it in Actions Secrets/Variables works out of the box | Optional |
...
| `TW_QUOTE_PRIORITY` | Taiwan (`.TW`/`.TWO`) daily K-line provider priority; lower values are tried earlier. Defaults to `6`, running as the Taiwan-only last fallback after YFinance (priority 4), built on official TWSE/TPEx monthly endpoints, requires no API key, and is always enabled. Does not affect the existing cn/hk/us/jp/kr fallback chains. | `6` | Optional |
...
| `MARKET_REVIEW_REGION` | Market review region: cn (A-shares), hk (HK stocks), us (US stocks), jp (JP stocks), kr (KR stocks), tw (Taiwan stocks), both (all six markets) | `cn` |
...
> Compatibility note (Issue #1815): `MARKET_REVIEW_REGION=cn|hk|us|jp|kr|tw|both` only expands the market set used by market review; `jp`/`kr`/`tw` are for recap scope and do not open JP/KR/TW for Market Light alerts.
...
- UI language is independent from generated-content language: `dsa.uiLanguage` (browser persistence) controls shell/login/settings text, while `REPORT_LANGUAGE` controls report text, report-page fixed copy, and Agent Chat replies that omit `context.report_language` (`zh`/`zh-tw`/`en`/`ko`).
...
> Note: `POST /api/v1/analysis/market-review` also accepts `report_language=zh|zh-tw|en|ko` / `reportLanguage` to set report language for that request. If omitted, it falls back to global `REPORT_LANGUAGE`; Bot/CLI/manual `/market-review` calls keep using global config and do not carry request-level override.
> Note: `POST /api/v1/analysis/market-review` accepts one optional 1–64 character string field, `region`. Valid inputs are `cn`, `hk`, `us`, `jp`, `kr`, `tw`, `both`, or a non-empty comma-separated subset such as `cn,us`. The request layer canonicalizes case, whitespace, duplicates, and ordering. Empty strings/tokens, unknown tokens, and mixing `both` with other markets reject the whole request with 4xx; they are never partially executed or silently defaulted. Omitting `region` continues to use global `MARKET_REVIEW_REGION`.
