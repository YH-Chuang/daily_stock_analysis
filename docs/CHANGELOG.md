...
- [修复] 分享图 `_POSTER_LABELS` 补齐 `zh-tw`，此前繁体海报外框是繁体、指标标签却回退成简体（现价／涨跌／止损／理想买入）
- [新功能] Web 前端支持 `zh-tw`：报告文案、市场复盘、运行诊断、数据上下文、市场结构卡、市场阶段与情绪标签均补齐繁体台湾用语，此前 `zh-tw` 报告会配上简体界面文案并在繁体正文上方重算出简体「乐观」
- [修复] `MARKET_REVIEW_REGION=both` 恢复为 `cn,hk,us,jp,kr`，不再包含 `tw`；此前 `tw` 被并入 `both`，所有既有 `both` 部署会在未改配置的情况下多跑一次台股复盘（多一次抓取与 LLM 调用）。需要台股复盘请显式写出 `tw`
- [修复] `TW_QUOTE_PRIORITY` 为空或非法值时退回默认优先级 6 并记录告警，不再在 import 期抛 `ValueError` 拖垮整条分析流程
- [测试] Telegram 分块回归改为断言粗体段落在投递文本中完整存活，此前的偶数星号断言由每块中和器保证成立，即使分块器退回固定位移切片也不会失败
