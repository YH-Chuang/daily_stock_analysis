...
// displayName 留空，交由 formatBrokerLabel 按界面语言补上券商名称。
...
const PortfolioPage: React.FC = () => {
  ...
  const loadBrokers = useCallback(async () => {
    try {
      ...
      if (brokerItems.length === 0) {
        ...
        setBrokerLoadWarning(t('portfolio.brokerListEmpty'));
      ...
      setBrokers(FALLBACK_BROKERS);
      setBrokerLoadWarning(t('portfolio.brokerListUnavailable'));
      if (!FALLBACK_BROKERS.some((item) => item.broker === selectedBroker)) {
        setSelectedBroker(FALLBACK_BROKERS[0].broker);
      }
    }
  ...
  const loadSnapshotAndRisk = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setRiskWarning(parsed.message || t('portfolio.riskLoadFailed'));
      }
    } catch (err) {
      setSnapshot(null);
      setRisk(null);
  ...
  const handleAnalyzePosition = async (row: FlatPosition) => {
    ...
    try {
      ...
      setPositionAnalysisMessage(t('portfolio.analysisSubmitted', {
        symbol: row.symbol,
        taskId: task.taskId,
      }));
    } catch (err) {
  ...
  const handleTradeSubmit = async (e: React.FormEvent) => {
    ...
    if (!writableAccountId) {
      setWriteWarning(t('portfolio.selectAccountForWrite'));
      return;
    }
    try {
      setWriteWarning(null);
  ...
  const openDeleteDialog = (item: PendingDelete) => {
    if (!writableAccountId) {
      setWriteWarning(t('portfolio.selectAccountForDelete'));
      return;
    }
  ...
  const openAccountDeleteDialog = () => {
    if (!writableAccount) {
      setWriteWarning(t('portfolio.selectAccountForAccountDelete'));
      return;
    }
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    if (!name) {
      setAccountCreateError(t('portfolio.accountNameRequired'));
    ...
    try {
      ...
      setAccountCreateSuccess(t('portfolio.accountCreateSuccessMessage'));
    } catch (err) {
      const parsed = getParsedApiError(err);
      setAccountCreateError(parsed.message || t('portfolio.accountCreateFailedMessage'));
      setAccountCreateSuccess(null);
    } finally {
  ...
  const handleRefreshFx = async () => {
    ...
    try {
      ...
      setFxRefreshFeedback(buildFxRefreshFeedback(result, t));
    } catch (err) {
      if (!isActiveRefreshContext(requestedViewKey, requestedRequestId)) {
        return;
      }
  ...
  return (
    <div className="portfolio-page min-h-screen space-y-4 p-4 md:p-6">
        ...
        <Card padding="md">
          <div className="flex items-center justify-between gap-2">
            ...
            {hasAccounts ? (
              ...
              <span className="text-xs text-secondary">{t('portfolio.autoSwitchHint')}</span>
            )}
          </div>
          {accountCreateError ? (
            <InlineAlert
              ...
              title={t('portfolio.accountCreateFailedTitle')}
          ...
          {accountCreateSuccess ? (
            <InlineAlert
              variant="success"
              className="mt-2 rounded-lg px-2 py-1 text-xs shadow-none"
              title={t('portfolio.accountCreateSuccessTitle')}
          ...
          <form className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-2" onSubmit={handleCreateAccount}>
            <input
              ...
              placeholder={t('portfolio.accountNamePlaceholder')}
              value={accountForm.name}
              onChange={(e) => setAccountForm((prev) => ({ ...prev, name: e.target.value }))}
            />
            <input
              className={PORTFOLIO_INPUT_CLASS}
              placeholder={t('portfolio.accountBrokerPlaceholder')}
              value={accountForm.broker}
              onChange={(e) => setAccountForm((prev) => ({ ...prev, broker: e.target.value }))}
            />
            <input
              className={PORTFOLIO_INPUT_CLASS}
              placeholder={t('portfolio.accountBaseCurrencyPlaceholder')}
            ...
            >
              <option value="cn">{t('portfolio.marketOptionCn')}</option>
              <option value="hk">{t('portfolio.marketOptionHk')}</option>
              <option value="us">{t('portfolio.marketOptionUs')}</option>
              <option value="jp">{t('portfolio.marketOptionJp')}</option>
              <option value="kr">{t('portfolio.marketOptionKr')}</option>
              <option value="tw">{t('portfolio.marketOptionTw')}</option>
            </select>
            <button type="submit" className="btn-secondary text-sm" disabled={accountCreating}>
              {accountCreating ? t('portfolio.creating') : text.createAccount}
            </button>
          </form>
        </Card>
      ) : null}
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{t('portfolio.manualTradeTitle')}</h3>
          <form className="space-y-2" onSubmit={handleTradeSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS} placeholder={t('portfolio.tradeSymbolPlaceholder')} value={tradeForm.symbol}
            ...
            <div className="grid grid-cols-2 gap-2">
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={t('portfolio.quantityPlaceholder')} value={tradeForm.quantity}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, quantity: e.target.value }))} required />
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={t('portfolio.pricePlaceholder')} value={tradeForm.price}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, price: e.target.value }))} required />
            </div>
            <div className="grid grid-cols-2 gap-2">
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={t('portfolio.feePlaceholder')} value={tradeForm.fee}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, fee: e.target.value }))} />
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={t('portfolio.taxPlaceholder')} value={tradeForm.tax}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, tax: e.target.value }))} />
            </div>
            <p className="text-xs text-secondary">{t('portfolio.feeTaxHint')}</p>
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>{t('portfolio.submitTrade')}</button>
          </form>
        </Card>

        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{t('portfolio.manualCashTitle')}</h3>
          <form className="space-y-2" onSubmit={handleCashSubmit}>
            <div className="grid grid-cols-2 gap-2">
              ...
              <select className={PORTFOLIO_SELECT_CLASS} value={cashForm.direction}
                ...
                <option value="in">{t('portfolio.cashIn')}</option>
                <option value="out">{t('portfolio.cashOut')}</option>
              </select>
            </div>
            <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder={t('portfolio.amountPlaceholder')}
              value={cashForm.amount} onChange={(e) => setCashForm((prev) => ({ ...prev, amount: e.target.value }))} required />
            <input
              className={PORTFOLIO_INPUT_CLASS}
              placeholder={t('portfolio.currencyPlaceholder', {
                currency: writableAccount?.baseCurrency || t('portfolio.accountBaseCurrency'),
              })}
              value={cashForm.currency}
              onChange={(e) => setCashForm((prev) => ({ ...prev, currency: e.target.value }))}
            />
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>{t('portfolio.submitCash')}</button>
          </form>
        </Card>

        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{t('portfolio.manualCorporateTitle')}</h3>
          <form className="space-y-2" onSubmit={handleCorporateSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS} placeholder={t('portfolio.symbolPlaceholder')} value={corpForm.symbol}
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <select className={PORTFOLIO_SELECT_CLASS} value={corpForm.actionType}
                ...
                <option value="cash_dividend">{t('portfolio.cashDividend')}</option>
                <option value="split_adjustment">{t('portfolio.splitAdjustment')}</option>
              </select>
            </div>
            {corpForm.actionType === 'cash_dividend' ? (
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.000001" placeholder={t('portfolio.dividendPerSharePlaceholder')}
              ...
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.000001" placeholder={t('portfolio.splitRatioPlaceholder')}
                value={corpForm.splitRatio}
                onChange={(e) => setCorpForm((prev) => ({ ...prev, splitRatio: e.target.value, cashDividendPerShare: '' }))} required />
            )}
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>{t('portfolio.submitCorporate')}</button>
          </form>
        </Card>
      </section>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-2 gap-3">
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{t('portfolio.csvImportTitle')}</h3>
          <div className="space-y-2">
            {brokerLoadWarning ? (
              <InlineAlert
                variant="warning"
            ...
            <div className="grid grid-cols-2 gap-2">
              <select className={PORTFOLIO_SELECT_CLASS} value={selectedBroker} onChange={(e) => setSelectedBroker(e.target.value)}>
                {brokers.length > 0 ? (
                  brokers.map((item) => <option key={item.broker} value={item.broker}>{formatBrokerLabel(item.broker, t, item.displayName)}</option>)
                ) : (
                  <option value="huatai">{formatBrokerLabel('huatai', t)}</option>
                )}
              </select>
              <label className={PORTFOLIO_FILE_PICKER_CLASS}>
                {t('portfolio.chooseCsv')}
            ...
            <div className="flex items-center gap-2 text-xs text-secondary">
              ...
              <label htmlFor="csv-dry-run">{t('portfolio.dryRunOnly')}</label>
            </div>
            <div className="flex gap-2">
              <button type="button" className="btn-secondary flex-1" disabled={!csvFile || csvParsing} onClick={() => void handleParseCsv()}>
                {csvParsing ? t('portfolio.parsing') : t('portfolio.parseFile')}
              </button>
              <button type="button" className="btn-secondary flex-1"
                disabled={!csvFile || !writableAccountId || csvCommitting} onClick={() => void handleCommitCsv()}>
                {csvCommitting ? t('portfolio.committing') : t('portfolio.submitImport')}
              </button>
            </div>
            {csvParseResult ? (
              <InlineAlert
                variant={getCsvParseVariant(csvParseResult)}
                title={t('portfolio.csvParseResultTitle')}
                message={t('portfolio.csvParseResultMessage', {
            ...
            {csvCommitResult ? (
              <InlineAlert
                ...
                title={csvDryRun ? t('portfolio.csvDryRunResultTitle') : t('portfolio.csvCommitResultTitle')}
                message={t('portfolio.csvCommitResultMessage', {
                  mode: csvDryRun ? t('portfolio.csvDryRunMode') : t('portfolio.csvCommitMode'),
        ...
        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">{t('portfolio.eventsTitle')}</h3>
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <select className={PORTFOLIO_SELECT_CLASS} value={eventType} onChange={(e) => setEventType(e.target.value as EventType)}>
                <option value="trade">{t('portfolio.eventTypeTrade')}</option>
                <option value="cash">{t('portfolio.eventTypeCash')}</option>
                <option value="corporate">{t('portfolio.eventTypeCorporate')}</option>
              </select>
              <button type="button" className="btn-secondary text-sm" onClick={() => void loadEvents()} disabled={eventLoading}>
                {eventLoading ? t('portfolio.loadingEvents') : t('portfolio.refreshEvents')}
              </button>
            </div>
            <div className="grid grid-cols-2 gap-2">
            ...
            {(eventType === 'trade' || eventType === 'corporate') ? (
              <input className={PORTFOLIO_INPUT_CLASS} placeholder={t('portfolio.filterBySymbolPlaceholder')} value={eventSymbol}
            ...
            {eventType === 'trade' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventSide} onChange={(e) => setEventSide(e.target.value as '' | PortfolioSide)}>
                <option value="">{t('portfolio.allSides')}</option>
                <option value="buy">{t('portfolio.buy')}</option>
                <option value="sell">{t('portfolio.sell')}</option>
              </select>
            ) : null}
            {eventType === 'cash' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventDirection}
                ...
                <option value="">{t('portfolio.allCashDirections')}</option>
                <option value="in">{t('portfolio.cashIn')}</option>
                <option value="out">{t('portfolio.cashOut')}</option>
              </select>
            ) : null}
            {eventType === 'corporate' ? (
              <select className={PORTFOLIO_SELECT_CLASS} value={eventActionType}
                ...
                <option value="">{t('portfolio.allCorporateActions')}</option>
                <option value="cash_dividend">{t('portfolio.cashDividend')}</option>
                <option value="split_adjustment">{t('portfolio.splitAdjustment')}</option>
              </select>
            ) : null}
            <div className="text-[11px] text-secondary">
              {writeBlocked ? t('portfolio.deleteScopeBlocked') : t('portfolio.deleteHint')}
            </div>
            <div className="max-h-64 overflow-auto rounded-lg border border-white/10 p-2">
              {eventType === 'trade' && tradeEvents.map((item) => (
                <div key={`t-${item.id}`} className="flex items-start justify-between gap-3 border-b border-white/5 py-2 text-xs text-secondary">
                  <div className="min-w-0">
                    {t('portfolio.eventTradeSummary', {
                      date: item.tradeDate,
                      side: formatSideLabel(item.side, t),
                      symbol: item.symbol,
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'trade',
                        id: item.id,
                        message: t('portfolio.confirmDeleteTrade', {
                          date: item.tradeDate,
                          side: formatSideLabel(item.side, t),
                          symbol: item.symbol,
                          quantity: item.quantity,
              ...
              {eventType === 'cash' && cashEvents.map((item) => (
                <div key={`c-${item.id}`} className="flex items-start justify-between gap-3 border-b border-white/5 py-2 text-xs text-secondary">
                  <div className="min-w-0">
                    {item.eventDate} {formatCashDirectionLabel(item.direction, t)} {item.amount} {item.currency}
                  </div>
                  {!writeBlocked ? (
                    <button
                      type="button"
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'cash',
                        id: item.id,
                        message: t('portfolio.confirmDeleteCash', {
                          date: item.eventDate,
                          direction: formatCashDirectionLabel(item.direction, t),
                          amount: item.amount,
              ...
              {eventType === 'corporate' && corporateEvents.map((item) => (
                <div key={`ca-${item.id}`} className="flex items-start justify-between gap-3 border-b border-white/5 py-2 text-xs text-secondary">
                  <div className="min-w-0">
                    {item.effectiveDate} {formatCorporateActionLabel(item.actionType, t)} {item.symbol}
                  </div>
                  {!writeBlocked ? (
                    <button
                      type="button"
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'corporate',
                        id: item.id,
                        message: t('portfolio.confirmDeleteCorporate', {
                          date: item.effectiveDate,
                          action: formatCorporateActionLabel(item.actionType, t),
                          symbol: item.symbol,
                        }),
                      })}
                    >
              ...
              {!eventLoading
                && ((eventType === 'trade' && tradeEvents.length === 0)
                  ...
                  || (eventType === 'corporate' && corporateEvents.length === 0)) ? (
                    <EmptyState
                      title={t('portfolio.noEventsTitle')}
                      description={t('portfolio.noEventsDescription')}
                      className="border-none bg-transparent px-3 py-6 shadow-none"
                    />
                  ) : null}
            </div>
            <div className="flex items-center justify-between text-xs text-secondary">
              <span>{t('portfolio.pageIndicator', { page: eventPage, pages: totalEventPages })}</span>
              <div className="flex gap-2">
      ...
      <ConfirmDialog
        ...
        title={t('portfolio.deleteEventTitle')}
        message={pendingDelete?.message || t('portfolio.deleteEventDefaultMessage')}
        confirmText={deleteLoading ? t('portfolio.deleting') : t('portfolio.confirmDelete')}
